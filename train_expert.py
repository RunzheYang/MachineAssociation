...
parser.add_argument('--test-size', type=int, default=10000, metavar='VS',
                    help='size of validation set')
...
val_loader = torch.utils.data.DataLoader(
    ...
    sampler=chunk.Chunk(args.val_size, train_size))

test_loader = torch.utils.data.DataLoader(
    dataset,
    batch_size=args.test_batch,
    sampler=chunk.Chunk(args.test_size, train_size + args.val_size))
...
for epoch in range(args.epochs):
    for data, target in train_loader:
        ...
        if cnt % 1000 == 0:
            ...
            val_acc = 100. * val_acc / args.val_size
